...
import { IRequestGiveFeedbackHomeworkEscola } from "../Interface/IRequestGiveFeedbackEscola";
import { IRequestAnswerHomeworkEscola } from "../Interface/IRequestAnswerHomeworkEscola";
import { IRequestGiveGradeHomeworkEscola } from "../Interface/IRequestGiveGradeEscola";

@injectable()
class HomeworkValidateEscola implements IHomeworkServiceStrategy {
    constructor(
    ...
    async validateCreate({name, details, creator_id}: ICreateHomeworkEscolaDTO): Promise<boolean> {
    ...
    async validateGiveFeedback({homework_id, team_id, feedback, professor_id}: IRequestGiveFeedbackHomeworkEscola): Promise<boolean> {
    ...
    async validateAnswerHomework({homework_id, team_id, answer}: IRequestAnswerHomeworkEscola): Promise<boolean> {
    ...
    async validateGiveGrade({homework_id, team_id, grade, professor_id}: IRequestGiveGradeHomeworkEscola): Promise<boolean> {
        const homework = await this.homeworkRepository.findById(homework_id);
        if(!homework){
            throw new AppError("Invalid Homework!", 400)
        }
